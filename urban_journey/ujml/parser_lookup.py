...
class UJMLLookup(etree.PythonElementClassLookup):
    ...
    def lookup(self, document, element):
        ...
        if is_default_namespace(element.tag):
            element_type = self.ujml_lookup(document, element)
            self.ujml_read_elements[get_path(element)] = element_type
            return element_type
        else:
            return None

    def ujml_lookup(self, document, element):
        ...
        # Look for a parent
        while not paths[-1] in self.ujml_read_elements:
            elements.append(elements[-1].getparent())
            paths.append(get_path(elements[-1]))

        for i in reversed(range(len(elements)-1)):
            klass = self.ujml_read_elements[paths[i+1]].lookup_child(self.filename, elements[i])
            if klass is None:
                if elements[i].tag in element_type_registry:
                    self.ujml_read_elements[paths[i]] = element_type_registry[elements[i].tag]
                else:
                    self.ujml_read_elements.pop(paths[i], None)
                    # Raise an error if the element was not found.
                    raise UnknownElementError(self.filename, element.sourceline, elements[i].tag)
            else:
                self.ujml_read_elements[paths[i]] = klass
        return self.ujml_read_elements[paths[0]]
