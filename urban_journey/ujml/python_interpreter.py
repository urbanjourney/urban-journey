...
class UJMLPythonInterpreterClass(object):
    ...
    def __getitem__(self, key):
        return self.__ujml_locals[key]

    def __setitem__(self, key, value):
        self.__ujml_locals[key] = value

    def exec(self, source, filename, lineno):
        exec(compile('\n'*(lineno-1)+source, filename, 'exec'), self.__ujml_locals)

    def run_src_object(self, src_obj):
        if isinstance(src_obj, UJMLPythonSourceClass):
            return eval(src_obj.code, self.__ujml_locals)
        else:
    ...
    def eval(self, source, filename, lineno):
        ...
        if len(lines) > 1:
            exec(compile('\n'*(lineno-1)+'\n'.join(lines[:-1]), filename, 'exec'), self.__ujml_locals)
        return eval(compile('\n'*(lineno-1+len(lines)-1)+lines[-1], filename, 'eval'), self.__ujml_locals)
